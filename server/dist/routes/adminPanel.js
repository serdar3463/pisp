...
router.get("/stats", (_req, res) => {
    const uniqueDevices = db_1.default.prepare("SELECT COUNT(DISTINCT device_hash) as c FROM offer_acceptances").get().c;
    const totalAcceptances = db_1.default.prepare("SELECT COUNT(*) as c FROM offer_acceptances").get().c;
    const pendingWithdrawals = db_1.default.prepare("SELECT COUNT(*) as c FROM withdrawal_requests WHERE status IN ('pending','processing')").get().c;
    const completedWithdrawals = db_1.default.prepare("SELECT COUNT(*) as c, COALESCE(SUM(estimated_try),0) as total FROM withdrawal_requests WHERE status='completed'").get();
    const payments = db_1.default.prepare("SELECT COUNT(*) as c, COALESCE(SUM(price_try),0) as total FROM user_payments WHERE status='completed'").get();
    res.json({
        uniqueDevices,
        totalAcceptances,
        pendingWithdrawals,
...
router.patch("/withdrawals/:id", (req, res) => {
    ...
    db_1.default.prepare("UPDATE withdrawal_requests SET status=?, processed_at=? WHERE id=?")
